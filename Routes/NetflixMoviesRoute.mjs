...
router.get("/seed", async (req, res) => {
  try {
    ...
    await NetflixMovies.insertMany(movies);
...
router.route('/')
  ...
  router
      ...
      .put(async(req,res)=>{
          try{
            let updateUser = await NetflixMovies.findByIdAndUpdate(req.params.id, req.body, {new: true});
            res.json(updateUser); 
  ...
  //delete
      .delete(async(req,res)=>{
        try{
           let deleteUser = await NetflixMovies.findByIdAndDelete(req.params.id);
           res.json(deleteUser);
        }catch(err){
            console.error(err);
            res.status(500).json({msg: `Error - ${err.message }`});
